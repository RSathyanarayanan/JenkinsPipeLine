...
import webPages.AdminACEPatientPage;
import webPages.AdminLoginPage;

public class AdminACEPatientScenarios extends ApplicationKeywords {
	BaseClass obj;
	private Boolean status = false;
	
	AdminLoginPage adminLoginPage;
	AdminACEPatientPage  adminACEPatientPage;
	AdminACEDiagnosticPage adminACEDiagnosticPage;

	
	public AdminACEPatientScenarios(BaseClass obj) {
		super(obj);
		this.obj = obj;
	}

	// WEB_IND_016
		public void verifyACEPatientPageNavigationandPatientPageUIElements() {
			try {
				
				adminLoginPage = new AdminLoginPage(obj);
				adminACEPatientPage = new AdminACEPatientPage(obj);
				...
				adminACEPatientPage.validateACEPatientPageNavigationandPatientPageUIElements();
				adminLoginPage.LogOutofTheAdminApplication();			
				
			} catch (Exception e) {
				System.err.println(e.toString());
			} finally {
				if (this.testFailure || adminLoginPage.testFailure || adminACEPatientPage.testFailure) {
					status = true;
					testFailure = status;
				}
			}
